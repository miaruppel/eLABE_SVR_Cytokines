function results = tenFOLD_svm_scripts_matlab(corrmat, labels, ind_test_set_corrmat, ind_test_set_labels, num_iter)
...
for n = 1:numIterations
    ...
    for cv = 1:10 % loop throug cross-validation folds
        ...
        % TRAINING/TESTING
        temp = svm_scripts_v2021_matlab(corrmat(:,:,idx_nonNaN(train_set{n,cv})),labels(idx_nonNaN(train_set{n,cv})),0,corrmat(:,:,idx_nonNaN(test_set{n,cv})),labels(idx_nonNaN(test_set{n,cv})),2);
